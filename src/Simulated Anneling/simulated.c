...
{
		char opt;
		bool time_input = FALSE;
		while((opt = getopt(argc, argv, ":b:c:d:g:r:s:t:l:vw:")) != -1)
    {
        switch(opt)
        {
            case 'b': // BEST_LOG
                strcpy(SaArqBest, optarg);
								SA_GRAVA_BEST = TRUE;
                break;
            case 'c': // CONFIG
                ...
                break;
	              ...
	              break;
						case 'v': // verbose prob
	              SA_VERBOSE_PROB = TRUE;
	              break;
						case 't': // Time
								TIME_CAN_SIMULATED = atof(optarg);
	              // strcpy(SaArqTempos, optarg);
								// time_input = TRUE;
	              break;
						case 's': // Simulation
